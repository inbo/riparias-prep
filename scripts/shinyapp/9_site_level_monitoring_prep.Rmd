...
monitoring_data <- monitoring_data%>%
  ...
  mutate(DAFOR = replace(DAFOR, DAFOR=="Abundant / Dominant (depending on quadrat)" , "Abundant (25-50%)" ))%>%
  mutate(DAFOR = replace(DAFOR, DAFOR=="Frequent to Dominant" , "Abundant (25-50%)" ))%>%
...
monitoring_data_summary <- monitoring_data %>%
  dplyr::select(Time_period, `Invasive species`, DAFOR)%>%
...
#export sub_data
write.csv(monitoring_data_summary, '~/GitHub/riparias-prep/data/interim/dafor_monitoring.csv', row.names=FALSE)
```