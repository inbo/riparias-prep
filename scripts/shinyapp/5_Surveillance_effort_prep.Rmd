...
summary_morethan10 <- summary%>%
  filter(Totobservations>10)

#create layer with only these cells####
EEA_high_search_effort <- EEA_1km_Riparias%>%
  filter(CELLCODE%in%summary_morethan10$CELLCODE)

#test_outcome
map <- leaflet()%>%
  addTiles()%>%
  addPolygons(data=EEA_high_search_effort)%>%
  addPolygons(data=RBU)
