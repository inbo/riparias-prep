...
koppen_geiger_shapes <- dir(path = "./data/spatial/koppen-geiger", pattern = ".tif")
koppen_geiger_shapes <- unique(koppen_geiger_shapes)

for(f in koppen_geiger_shapes){
  shape <- raster(paste0("./data/spatial/koppen-geiger/", f))
  shape2 <- sp::as.SpatialPolygons.GridTopology(shape)
  test <- getValues(shape)
